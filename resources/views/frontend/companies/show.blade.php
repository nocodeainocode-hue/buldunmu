    ...
    <div class="mx-auto grid gap-8 px-4 py-10 sm:px-6 lg:grid-cols-[1fr_340px] lg:px-8" style="max-width:var(--page_width,1280px);">
        ...
        <main class="space-y-8">
            ...
            @if($company->images && $company->images->isNotEmpty())
                <section class="rounded-3xl border bg-white p-6" style="border-color:var(--border);box-shadow:var(--card_shadow);">
                    <h2 class="mb-5 text-2xl font-black" style="color:var(--text);">📸 Fotoğraf Galerisi</h2>
                    <div class="gallery-grid grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
                        @foreach($company->images as $index => $image)
                            <div class="gallery-item aspect-square overflow-hidden rounded-2xl cursor-pointer relative group border" style="border-color:var(--border);" data-index="{{ $index }}" data-src="{{ asset('storage/' . $image->image_path) }}">
                                <img src="{{ asset('storage/' . $image->image_path) }}" alt="{{ $company->name }} - Fotoğraf {{ $index + 1 }}" class="h-full w-full object-cover transition duration-300 group-hover:scale-110" loading="lazy">
            ...
            <section class="rounded-3xl border bg-white p-6" style="border-color:var(--border);box-shadow:var(--card_shadow);">
                <h2 class="mb-5 text-2xl font-black" style="color:var(--text);">📍 Konum ve Yol Tarifi</h2>
                <div class="relative w-full overflow-hidden rounded-2xl border" style="padding-top:56.25%;border-color:var(--border);">
                    <iframe
                ...
                @if($company->address)
                    <p class="mt-3 text-sm" style="color:var(--text_muted);">📫 {{ $company->address }}</p>
                @endif
            </section>
            @endif
